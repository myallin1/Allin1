...
export const revokeCouponOnServiceDeleted = functions.firestore
  ...
  .onDelete(async (snap, context) => {
    ...
    try {
      await db.runTransaction(async (tx) => {
        ...
        const status = couponSnap.data()?.status as string | undefined;
        if (status !== 'awaiting_gift' && status !== 'ready') {
          logger.info(
            `[revokeCouponOnServiceDeleted] coupon ${requestId} already in '${status}' — leaving it alone`,
          );
          return;
        }
        tx.update(couponRef, {
      ...
      logger.error(
        `[revokeCouponOnServiceDeleted] revoke failed for ${requestId}:`,
        error?.message ?? error,
      );
    }
    return null;
...
export const armStaleGiftCoupons = functions.pubsub
  ...
  .onRun(async () => {
    ...
    try {
      ...
      if (data && typeof data.value === 'number') {
        fallback = {
          giftType: (data.giftType as string) ?? 'discount',
          value: data.value,
      ...
      logger.warn(
        '[armStaleGiftCoupons] could not read defaults, using hardcoded:',
        error?.message ?? error,
      );
    }
    ...
    if (stale.empty) {
      logger.info('[armStaleGiftCoupons] nothing stale');
      return null;
    }
    ...
    logger.info(`[armStaleGiftCoupons] auto-armed ${stale.size} coupon(s)`);
    return null;
  });
