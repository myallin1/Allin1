...
exports.phonepeWebhook = functions.https.onRequest(async (req, res) => {
    ...
    const base64Response = (_a = req.body) === null || _a === void 0 ? void 0 : _a.response;
    ...
    const expectedChecksum = crypto.createHash('sha256').update(base64Response + SALT_KEY).digest('hex') +
        '###' +
        SALT_INDEX;
    if (receivedChecksum !== expectedChecksum) {
        firebase_functions_1.logger.warn('phonepeWebhook: checksum mismatch — rejecting callback', {
            receivedChecksum,
        });
    ...
    const { merchantTransactionId, state, transactionId, amount } = payload.data || {};
    ...
    try {
        ...
        const outcome = await db.runTransaction(async (transaction) => {
            ...
            if (typeof amount === 'number' && amount !== order.amountPaise) {
                firebase_functions_1.logger.error('phonepeWebhook: amount mismatch', {
        ...
        firebase_functions_1.logger.info('phonepeWebhook processed', { merchantTransactionId, outcome });
        // PhonePe only checks HTTP status, not body — 200 tells it to stop retrying.
        res.status(200).json({ success: true });
    }
    catch (error) {
        if (error.message === 'ORDER_NOT_FOUND') {
            firebase_functions_1.logger.error(`phonepeWebhook: no payment_orders doc for ${merchantTransactionId}`);
        ...
        firebase_functions_1.logger.error('phonepeWebhook failed:', error);
        res.status(500).json({ success: false, message: 'Internal error' });
    }
});
//# sourceMappingURL=phonepeWebhook.js.map