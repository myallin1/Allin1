...
class _HeroBuddyFormScreenState extends State<HeroBuddyFormScreen> {
  ...
  void _addTaskField() => setState(() => _taskControllers.add(TextEditingController()));
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton.icon(
            style: ElevatedButton.styleFrom(backgroundColor: _coPink, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
            onPressed: () async {
              Navigator.pop(ctx);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: _coBg, elevation: 0,
        leading: const BackButton(color: Colors.white),
        title: Text('Multi-Task Booking', style: GoogleFonts.outfit(color: Colors.white, fontWeight: FontWeight.w800)),
      ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          ..._taskControllers.asMap().entries.map((e) => _buildTaskField(e.key, e.value)),
          Align(
            alignment: Alignment.centerRight,
            child: TextButton.icon(
              onPressed: _addTaskField,
              icon: const Icon(Icons.add_circle_outline, color: _coPink, size: 18),
              label: Text('Add Another Item/Task', style: GoogleFonts.outfit(color: _coPink, fontWeight: FontWeight.w700)),
            ),
          ),
          const SizedBox(height: 20),
          _buildSectionTitle('Special Instructions'),
          TextField(
            controller: _instructionCtrl, maxLines: 3, style: const TextStyle(color: Colors.white),
            decoration: _inputDeco('Any shop name, brand preference, or warnings?'),
          ),
          const SizedBox(height: 24),
          _buildSectionTitle('Image Attachment'),
          Container(
            width: double.infinity, padding: const EdgeInsets.all(24),
            decoration: BoxDecoration(color: _coSurface, borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.white12)),
            child: Column(
              children: [
  ...
  Widget _buildSectionTitle(String title) => Padding(padding: const EdgeInsets.only(bottom: 12, top: 8), child: Text(title, style: GoogleFonts.outfit(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w700)));
  ...
  InputDecoration _inputDeco(String hint) => InputDecoration(hintText: hint, hintStyle: const TextStyle(color: Colors.white38, fontSize: 13), filled: true, fillColor: _coSurface, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: _coPink)));

  Widget _buildBottomBar() {
    return Container(
      padding: const EdgeInsets.all(16), decoration: const BoxDecoration(color: _coSurface, border: Border(top: BorderSide(color: _coBorder))),
      child: SafeArea(
        child: Row(
          children: [
  ...
  Widget _buildSearchingUI() => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const CircularProgressIndicator(color: _coPink), const SizedBox(height: 24), Text('Finding a local Hero for you...', style: GoogleFonts.outfit(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w700)), const SizedBox(height: 8), Text('Usually takes less than 3 minutes', style: GoogleFonts.outfit(color: Colors.white54, fontSize: 13))]));
}
