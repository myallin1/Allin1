...
class AdminIncomingCallDialog extends StatefulWidget {
  ...
  @override
  State<AdminIncomingCallDialog> createState() =>
      _AdminIncomingCallDialogState();
}
...
class _AdminIncomingCallDialogState extends State<AdminIncomingCallDialog>
  ...
  Future<void> _answerHuman() async {
    await ChittiLiveCallService.instance
        .answerCallHuman(_state.callId, adminId: widget.adminId);
  ...
  Future<void> _answerChitti() async {
    await ChittiLiveCallService.instance
        .answerCallChitti(_state.callId, adminId: widget.adminId);
  }

  Future<void> _takeOver() async {
    await ChittiLiveCallService.instance
        .takeOverCall(_state.callId, adminId: widget.adminId);
    if (_state.callerPhone.isNotEmpty) {
      await Future<void>.delayed(const Duration(milliseconds: 300));
      final uri = Uri.parse('tel:${_state.callerPhone}');
      await launchUrl(uri, mode: LaunchMode.externalApplication);
  ...
  Widget build(BuildContext context) {
    ...
    final isHumanConnected =
        _state.status == 'connected' && _state.handlingMode == 'human';
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: isHumanConnected
                          ? _green
                          : (isChittiHandling ? _purple : _green),
                    ),
                  ),
                  child: Row(
