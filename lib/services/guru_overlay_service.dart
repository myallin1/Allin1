  ...
  String _confirmationTextFor(Map<String, dynamic> args) {
    switch (args['action'] as String?) {
      ...
      case 'propose_dev_plan':
        final title = (args['title'] as String?)?.trim() ?? '(untitled)';
        final desc = (args['description'] as String?)?.trim() ?? '';
        // NEW (Sep 18 2026): propose_dev_plan now takes the same
        // 'engine' arg create_dev_task does — say which one out loud,
        // same reasoning as create_dev_task above.
      ...
      case 'approve_dev_plan':
        ...
        // approve_dev_plan resolves the actual engine at execution time
        // via ChittiDevTaskService.readLastPlanEngine() (an async prefs
        // read) — this confirmation-text builder is synchronous, so it
        // deliberately stays engine-neutral here rather than guessing
        // wrong the way a hardcoded "Claude" did before.
