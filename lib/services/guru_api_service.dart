...
class GuruApiService {
  ...
  Future<String> sendMessage({
    ...
    final headers = chittiRequestHeaders(model: model, apiKey: apiKey);
  ...
  // model fix calls the exact same OAuth-vs-api-key detection
    ...
    switch (variant) {
      ...
      case 'admin':
        ...
        return 'You are Chitti, chief of staff to Nizam — the owner of '
            ...
            'WHICH ENGINE BUILDS IT. Both create_dev_task AND '
            'propose_dev_plan take an optional "engine" argument: claude '
            '(default — most reliable, use unless told otherwise), '
            'gemini, or antigravity. Only set it to gemini/antigravity '
            'when Nizam explicitly names that engine. Both share one '
            'free-tier Gemini API quota that a single real coding task '
            'can exhaust in under a minute — if he asks which to pick, '
            'say so plainly rather than promising it will work. '
            'approve_dev_plan always goes to whichever engine actually '
            'drafted that plan, automatically.\n'
